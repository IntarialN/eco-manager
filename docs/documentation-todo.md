...
| Риски | Синхронизация с требованиями и календарём | Статус риска обновляется в `RequirementController::actionUpdateStatus`, события календаря синхронизируются (см. `docs/features/calendar.md`). | Да — добавить тесты и обработку планов действий | Backend | Готово (unit-тесты `RequirementController`) |
| Документы | Управление файлами из карточки требования | Поддержан upload/approve/reject на странице требования; документация обновлена. | Да — добавить валидацию типов и роли | Backend | Готово (нужны тесты) |
