...
| Риски | Синхронизация с требованиями и календарём | Статус риска обновляется в `RequirementController::actionUpdateStatus`, события календаря синхронизируются (см. `docs/features/calendar.md`). | Да — подключить уведомления/эскалации после изменений плана | Backend | Готово (unit-тесты + план действий) |
