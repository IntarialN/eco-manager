...
- `RiskActionPlan`:
  ...
  - Статусы: `new` (не начато), `in_progress`, `done`
- `RiskLog`:
  - `risk_id`, `timestamp`, `user_id`, `action`, `notes`
  - Фиксирует добавление задач, смену статусов и эскалации
...
- План действий влияет на итоговый балл:
  - все задачи `done` → коэффициент вероятности ×0.8;
  - есть просроченные задачи → коэффициент ×1.15;
  - при отсутствии активного плана риск автоматически эскалируется (`status = escalated`) и уведомляет руководителя.

## 6. Интеграции

- Получает события из блока требований/календаря.
- План действий хранится в таблице `risk_action_plan` (см. миграцию `m230000_000004_create_risk_action_plan_table.php`) и использует общий UI чек-листов.
- Ссылается на документы из хранилища: подтверждённый документ автоматически снижает вероятность и может закрыть риск.
- Публикует статусы в ленту изменений и уведомления (`NotificationService`, email/чат-бот).
- Финансовые риски создаются при просрочке оплат в календаре (интеграция с mock Bubble API).

## 7. UX

- Дэшборд с плитками рисков (по критичности) + отдельная полка «Эскалированные».
- Диаграмма изменения рискового балла во времени и вкладка «План действий».
- Детальная карточка риска: источник, штраф, план действий, история, привязанные документы.
- План действий (inline-редактор) позволяет добавлять задачи, назначать ответственных, менять статус без перехода в другие разделы; просрочки подсвечиваются.
- «История операций» показывает `RiskLog` с комментариями и ссылками на документы/события.
- Фильтры по объектам, статусам, категориям НВОС, типу источника.
...
- Тесты на автоматическое закрытие риска после выполнения плана.
  - Unit-тест `tests/unit/RiskActionPlanControllerTest.php` покрывает сценарий добавления задач, журналирование и перевод риска в `closed`.
- Проверка прав доступа (клиент видит только свои риски).
- Интеграционные тесты с календарём, документами и mock Bubble API (финансовые риски).
- Нагрузочный тест на историю событий (`RiskLog`) ≥ 1000 записей.
