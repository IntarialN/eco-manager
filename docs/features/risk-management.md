...
- Отправляет события в `NotificationService` (email для менеджеров + лог, далее интеграция с notification-service из архитектуры).
...
- План действий позволяет добавлять задачи, назначать ответственных и менять статус без перехода в другие разделы.
- Отдельный блок «История операций» отображает журнал `RiskLog` (кто и когда добавил задачу, изменил статус, закрыл риск).
