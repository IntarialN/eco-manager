...
- Периодическая синхронизация статусов договоров/счетов (pull) каждые 6 часов. Используем mock Bubble API (`docs/architecture/mock-bubble-api.md`) с эндпоинтами:
  - `GET /contracts?updatedSince=...`
  - `GET /invoices?status=...`
  - `GET /acts?contractId=...`
- Поддерживаем дифф синхронизацию: сохраняем `integration_revision` и обновляем только изменённые записи.
- Webhook (push) на события оплаты/формирования акта: `POST /webhooks/payment` → обновляем `Invoice.status`, создаём событие календаря, снимаем риск «финансовая просрочка».
- TODO: после получения реальной спецификации зафиксировать аутентификацию (OAuth/API key), лимиты и маппинг дополнительных полей (ставка НДС, частичная оплата) в `profile-logic`.

## 5. Автоматизация

- При переходе статуса счёта в `overdue` создаётся напоминание в календаре и риск «финансовая просрочка».
- Завершение работы по требованию → появляется задача сформировать акт (план действий в блоке рисков).
...
- При получении оплаты автоматически обновляется связанный риск и уведомление (клиенту/финансовому менеджеру).
- Подтверждённый акт создаёт линк на артефакт в хранилище документов (Document ID).
...
- Для счёта доступен таймлайн оплаты: строка «выставлен / отправлен клиенту / оплачен / просрочен».
- Быстрые действия: «Отправить напоминание», «Сформировать акт», «Выгрузить PDF».
- В карточке договора отображается связь с требованиями и прогресс по актам/оплатам.
...
- Интеграционные e2e-тесты (mock Bubble ↔ календарь ↔ риски) в `services/risk-service-e2e/`.
- Проверка автогенерации событий и рисков при обновлении статусов.
