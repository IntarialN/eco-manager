...
- Договоры и финансы:
  - Счета на оплату (выставление, оплата, просрочка).
  - Акт/закрывающие документы (подписание, архивирование).
  - План-факт по оплатам интегрируется с mock Bubble API (статусы `issued`, `paid`, `overdue`).
- TODO: при интеграции с учётной системой уточнить дополнительные статусы (частичная оплата, аннулирование).
...
- `CalendarEvent`:
  ...
  - `custom_interval_days` (используется для `periodicity = custom`), `reminder_days` (JSON-строка с оффсетами)
...
- После подтверждения требования события с периодичностью `monthly/quarterly/yearly/custom` автоматически создают следующую запись с новой датой и статусом «Запланировано», что исключает ручной перенос дедлайнов.
...
- Для договорных событий используется cron-синхронизация: каждые 6 часов подтягиваются статусы счетов/актов из mock Bubble API, сравниваются с локальными событиями и создаются напоминания (например, «Счёт №123 не оплачен > 10 дней»).
- События документов автоматически создаются при выборе «с аудитом» и ждут подтверждения специалиста; после утверждения документом событие переводится в `done`.
...
- На карточке требования отображается мини-таймлайн предстоящих дат (следующие 5 событий) с быстрыми действиями («отметить выполненным», «перенести на N дней»).
- Фильтры по типам/объектам/ответственным синхронизированы с фильтрами блока требований и документами (единый компонент).
- Для платёжных событий добавлен индикатор суммы и дати последнего обновления статуса из учётной системы.
...
- Для договоров и оплат уведомления дублируются в канал финансового менеджера и клиента (если включена опция «share invoices»).
- Для событий, связанных с аудитом документов, добавлен отдельный шаблон: уведомление отправляется специалисту аудита и клиенту после принятия/отклонения.
...
- Интеграционные тесты e2e (папка `services/risk-service-e2e/`) проверяют, что mock Bubble API корректно синхронизирует статусы счетов → календарь, и что просроченные платежи создают риск-событие.
