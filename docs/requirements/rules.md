...
- Требование `req_11` (инструкции и приказы): подтверждено нормативами (ФЗ-89 «Об отходах…», Пост. Правительства №262). Инструкции обязательны, если у клиента фиксируется образование отходов (факт наличия отходов I–IV классов или ненулевой `annual_waste_kg`) и назначено хотя бы одно ответственное лицо. При необходимости админ может принудительно включить требование флагом `need_instruction_docs`.
...
- [x] Подтверждено требование `req_11` нормативными актами.
- [ ] Проведён аудит правил после получения подтверждённых данных от заказчика (ожидает).

## Связанные документы и порядок обновления

- `docs/requirements/catalog.md` — перечень требований и нормативная база.
- `docs/client/profile-logic.md` — маппинг анкеты на атрибуты, участвующие в правилах.
- `docs/admin/reference-management.md` — справочники НПА и overrides.
- `docs/features/requirements-tracker.md` — пользовательские сценарии и интерфейсная логика.
- `docs/features/calendar.md` / `docs/features/risk-management.md` — вторичные эффекты (события, риски).

Перед изменением правил:
1. Проверить, отражён ли новый атрибут или требование в анкете и каталоге.
2. Обновить связанные документы и TODO/Completed, чтобы не потерять контекст изменений.
3. Зафиксировать правки в `docs/completed/` отдельной записью с перечислением обновлённых файлов.
