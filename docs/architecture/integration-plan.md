...
- Для каждого типа события задаётся профиль уведомлений:
  - `requirement.expired` — email + чат-бот.
...
- `docs/architecture/mock-bubble-api.md` — контракт mock-сервиса Bubble и cron-сценарии.
- `docs/features/*.md` — функциональные блоки, использующие интеграции (календарь, риски, биллинг).
- `docs/admin/reference-management.md` — справочники, зависящие от внешних данных.
- `docs/infra/runbook.md` — шаги по настройке интеграций в окружениях.
- `docs/architecture/adr/*.md` — решения по стеку, инфраструктуре и переходам.

Перед изменением плана интеграций:
1. Свериться с актуальными ADR, контрактами mock/production и completed-логом.
2. Обновить связанные функциональные документы и TODO-таблицу.
3. Зафиксировать изменения записью в `docs/completed/`, при необходимости обновить диаграммы последовательностей.
