...
- **Формат**:
  - `GET /contracts?client_id=...` — список договоров.
  - `GET /invoices?contract_id=...` — список счетов.
  - `PATCH /invoices/{id}` — обновление статуса.
- **TODO**: дождаться спецификации от заказчика, обновить эндпоинты и поля.
