...
- Зафиксировать изменения контракта в `docs/architecture/integration-plan.md`, обновить TODO и completed-лог.

## Связанные документы и контроль изменений

- `docs/architecture/integration-plan.md` — общий план интеграций и последовательность внедрения.
- `docs/features/contracts-billing.md` — бизнес-логика работы с договорами/счетами/актами.
- `docs/features/calendar.md` и `docs/features/risk-management.md` — вторичные эффекты (напоминания, риски).
- `docs/infra/runbook.md` — инструкция по запуску mock-сервиса и настройке cron-задач.
- `docs/documentation-todo.md` — статус получения реальной спецификации и сопутствующих задач.
- `docs/completed/` — история обновлений по интеграции Bubble.

Перед изменением mock-контракта:
1. Убедиться, что изменения отражены в `integration-plan.md` и функциональных документах.
2. Обновить TODO/Completed, отметить требуемые действия по реализации.
3. При появлении новой архитектурной зависимости оформить ADR либо дополнить существующие решения.
